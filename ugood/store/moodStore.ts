...
export const useMoodStore = create<MoodState>()(
    persist(
        (set, get) => ({
            ...
            recordMood: async (userId: string, emoji: string) => {
                ...
                try {
                    ...
                    NotificationService.scheduleMoodReminder(frequency);
        ...
        {
            name: 'mood-storage',
            storage: createJSONStorage(() => AsyncStorage), // Correct storage for React Native
            partialize: (state) => ({ frequency: state.frequency, lastMoodAt: state.lastMoodAt }), // Persist frequency and lastMoodAt
        }
    )
);
